...
describe("Bill Delivery Activity", function(){  
    const userInfo = {
        "grant_type":"password",
        "username":"mapp004",
        "password":"test123",
    ...
    describe("Step 2", function(){
		it("Makes request updateBillDelivery", function(done){  
            request
                .put(`http://127.0.0.1:3000/apigateway`)
                .type('json')
                .set('service', 'billDelivery')
                .set('Content-Type', 'application/json')
                .set({sec02token: tokenSec})
                .send({barId: userInfo.barId, isPaperless: true})
                .end((err, httpResponse) => { 
                    console.log(`Bill Delivery Response ${JSON.stringify(httpResponse)}`)
                    expect(err).to.not.exist;
                    expect(httpResponse.status).to.equals(200);
                    done()
                });
    ...
    describe("Step 3", function(){
		it("Makes request to retrieveBillDelivery", function(done){  
            request
