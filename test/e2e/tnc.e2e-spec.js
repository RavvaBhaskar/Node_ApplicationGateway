...
describe("TnC Information", function(){  
    const userInfo = {
        "grant_type":"password",
        "username":"mapp004",
        "password":"test123",
    ...
    describe("Step 2", function(){
		it("Makes request updateTnc", function(done){  
            request
                .put(`http://127.0.0.1:3000/apigateway?checkTnC=${userInfo.tnc}`)
                .type('json')
                .set('service', 'accountInfo')
                .set('Content-Type', 'application/json')
                .set({sec02token: tokenSec})
                .end((err, httpResponse) => { 
                    console.log(`TnC Response ${JSON.stringify(httpResponse)}`)
                    expect(err).to.not.exist;
                    expect(httpResponse.status).to.equals(200);
                    done()
                });
    ...
    describe("Step 3", function(){
		it("Makes request retrieveTnc", function(done){  
            request
                .get(`http://127.0.0.1:3000/apigateway?checkTnC=${userInfo.tnc}`)
                .type('json')
                .set('service', 'accountInfo')
                .set('Content-Type', 'application/json')
                .set({sec02token: tokenSec})
