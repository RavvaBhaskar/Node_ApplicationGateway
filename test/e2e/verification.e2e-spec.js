...
describe("Verification Information", function(){  
    const userInfo = {
        grant_type:"password",
        username:"mapp004",
        password:"test123",
    ...
    var otokenVal;
    var tokenSec; 
    var ctn = {"value":`${JSON.parse(userInfo.ctn)}`};
    ...
    describe("Step 2", function(){
		it("Makes request getVerification", function(done){  
            request
                .get(`http://127.0.0.1:3000/apigateway?accountLookupVerification&ctn={"value":${userInfo.ctn}}`)
                .type('json')
                .set('service', 'verification')
                .set('Content-Type', 'application/json')
                .set('name', 'accountLookup')
                .set({sec02token: tokenSec})
                .end((err, httpResponse) => { 
                    console.log(`Verification information Response ${JSON.stringify(httpResponse)}`)
                    expect(err).to.not.exist;
                    expect(httpResponse.status).to.equals(200);
                    done()
                });
