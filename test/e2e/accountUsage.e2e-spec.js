...
describe.only("Account Usage Information", function(){  
   const userInfo = {
        "grant_type":"password",
        "username":"mapp004",
        "password":"test123",
    ...
    describe("Step 2", function(){
		it("Makes request to accountUsage", function(done){  
            request
                .get(`http://127.0.0.1:3000/apigateway?cid=${userInfo.cid}&faId=${userInfo.faId}`)
                .type('json')
                .set('service', 'accountUsage')
                .set('Content-Type', 'application/json')
                .set({sec02token: tokenSec})
                .end((err, httpResponse) => { 
                    console.log(`AccountUsage Response ${JSON.stringify(httpResponse)}`)
                    expect(err).to.not.exist;
                    expect(httpResponse.status).to.equals(200);
                    done()
                });
