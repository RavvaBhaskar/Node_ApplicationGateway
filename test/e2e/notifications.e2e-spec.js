...
describe("Notifications Activity", function(){  
    const userInfo = {
        "grant_type":"password",
        "username":"mapp004",
        "password":"test123",
    ...
    describe("Step 2", function(){
		it("Makes request setNotification", function(done){  
            request
                ...
                .send({notification:{
                    id: 1, 
                    label: "A new statement is avaiable", 
                    methods: [
                        {type: "email", isChecked: true, isRequired: false}, 
                        {type: "text", isChecked: false, isRequired: false}
                    ]}
                })
                .end((err, httpResponse) => { 
                    console.log(`Notifications Response ${JSON.stringify(httpResponse)}`)
                    expect(err).to.not.exist;
                    expect(httpResponse.status).to.equals(200);
                    done()
                });
    ...
    describe("Step 3", function(){
		it("Makes request to retrieveNotification", function(done){  
            request
