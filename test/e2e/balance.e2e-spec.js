...
describe("Balance Information", function(){  
    ...
    describe("Step 2", function(){
		it("Makes request retrieveBalance", function(done){  
            request
                .get(`http://127.0.0.1:3000/apigateway?ctn=${userInfo.ctn}`)
                .type('json')
                .set('service', 'balance')
                .set('Content-Type', 'application/json')
                .set({sec02token: tokenSec})
                .end((err, httpResponse) => { 
                    console.log(`Balance Response ${JSON.stringify(httpResponse)}`)
                    expect(err).to.not.exist;
                    expect(httpResponse.status).to.equals(200);
                    done()
                });
