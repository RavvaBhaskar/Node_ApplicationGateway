...
describe("Account Products Information", function(){  
    const userInfo = {
        "grant_type":"password",
        "username":"mapp004",
        "password":"test123",
    ...
    describe("Step 2", function(){
		it("Makes request retrieveAccountProducts", function(done){  
            request
                .get(`http://127.0.0.1:3000/apigateway`)
                .type('json')
                .set('service', 'accountProducts')
                .set('Content-Type', 'application/json')
                .set({sec02token: tokenSec})
                .end((err, httpResponse) => { 
                    console.log(`Account Products Response ${JSON.stringify(httpResponse)}`)
                    expect(err).to.not.exist;
                    expect(httpResponse.status).to.equals(200);
                    done()
                });
